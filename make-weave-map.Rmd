...
hex_unit <- ## hex example
  get_triaxial_weave_unit(width = 200, spacing = 600, margin = 2,
...
```{r}
cube_unit <-
  get_triaxial_weave_unit(spacing = 500, ids = "ab|cd|ef", margin = 5, 
...
diamond_unit <-   ## diamond example
  get_triaxial_weave_unit(width = 200, spacing = 600, margin = 10,
...
[Online full screen version](https://dosull.github.io/tiling-space/example-map-1.html)
...
weave_maps <- list()
widths <- c(250, 500, 750)
for (i in 1:length(widths)) {
  weave <- get_triaxial_weave_unit(width = widths[i], type = "hex", crs = st_crs(welly.votes)) %>%
...
+ You'll find code at [github.com/DOSull/weaving-space](https://github.com/DOSull/tiling-space)
