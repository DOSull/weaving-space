...
rect11_unit$primitive %>% plot(border = NA)
```
...
rect32_unit$primitive %>% plot(border = NA)
```
...
rect34_unit$primitive %>% plot(border = NA)
```
...
twill_unit$primitive %>% plot(border = NA)
```
...
basket_unit <- 
  get_biaxial_weave_unit(spacing = 200, type = "basket", n = 2, 
                         aspect = 0.6, ids = "ab|cd", crs = st_crs(region))
basket_unit$primitive %>% plot(border = NA)
```

### Other weaves
We can generate any weavable repeat pattern, even crazy ones!

```{r warning = FALSE}
this_unit <- 
  get_biaxial_weave_unit(spacing = 200, type = "this", ids = ids,
                        crs = st_crs(region))
this_unit$primitive %>% plot(border = NA)
```
...
hex_unit$primitive %>% plot(border = NA)
```
...
```{r}
twill_unit$primitive %>% sf_square_to_diamond() %>% plot(border = NA)
```
...
## References
Glassner, A. 2002. [Digital Weaving 1](https://dx.doi.org/10.1109/MCG.2002.1046635). IEEE Computer Graphics and Applications 22 (6):108–118.

———. 2003a. [Digital Weaving 2](https://dx.doi.org/10.1109/MCG.2003.1159616). IEEE Computer Graphics and Applications 23 (1):77–90.

———. 2003b. [Digital Weaving 3](https://dx.doi.org/10.1109/MCG.2003.1185583). IEEE Computer Graphics and Applications 23 (2):80–83.
...
And this video provided a nice way in to the topic for us: ["It's Just Matrix Multiplication": Notation for Weaving](https://www.youtube.com/watch?v=oMOSiag3dxg) by Lea Albaugh, presented at the Strange Loop conference, St Louis, 27-28 Sept, 2018. 
