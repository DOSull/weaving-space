...
class Tileable:
  ...
  def merge_fragments(self, frags:list[geom.Polygon]) -> list[geom.Polygon]:
    """
    ...
    Args:
      frags (list[geom.Polygon]): A set of polygons to merge.

    Returns:
      list[geom.Polygon]: A minimal list of merged polygons.
    """
    if len(frags) == 1:
      return [f for f in frags if not f.is_empty]
    frags = [f for f in frags if not f.is_empty]
    tile = self.tile.geometry[0]
    reg_tile = copy.deepcopy(self.regularised_tile.geometry[0])
    changes_made = True
    while changes_made:
      changes_made = False
      for v in self.vectors:
        ...
        next_frags = []
        t_frags = [affine.translate(f, v[0], v[1]) for f in frags]
        ...
        frags = [f for i, f in enumerate(frags)
                 if not (i in frags_to_remove)]
        frags = next_frags + frags
    self.regularised_tile.geometry[0] = reg_tile
    return [f for f in frags if not f.is_empty] # don't return any duds


  def reattach_elements(self) -> None:
    """Move elements that are outside the regularised tile main polygon
    ...
    new_elements = list(self.elements.geometry)
    for i, p in enumerate(self.elements.geometry):
    ...
    self.regularised_tile.geometry[0] = new_reg_tile
    return None
  ...
  def regularise_elements(self) -> None:
    ...
    self.regularised_tile.geometry = self.regularised_tile.geometry.explode(
      index_parts=False, ignore_index=True)
    if self.regularised_tile.geometry.shape[0] > 1:
      self.regularised_tile.geometry = tiling_utils.get_largest_polygon(
  ...
  def get_local_patch(self, r: int = 1,
    ...
    if not include_0:  # throw away the identity vector
      vecs.pop((0, 0, 0) if self.tile_shape in (TileShape.HEXAGON,) else (0, 0))
    ids, tiles = [], []
    for v in vecs.values():
      ids.extend(self.elements.element_id)
      tiles.extend(
        self.elements.geometry.apply(affine.translate, xoff=v[0], yoff=v[1])
      )
    return gpd.GeoDataFrame(
      data={"element_id": ids}, crs=self.crs, geometry=gpd.GeoSeries(tiles)
    )
  ...
  def fit_elements_to_tile(self, centre_element: int = 0) -> None:
    ...
    self.elements.geometry = tiling_utils.clean_polygon(self.elements.geometry)
  ...
  def plot(
    ...
    ax=None,
    show_tile: bool = True,
    show_reg_tile: bool = True,
    show_ids: str = "element_id",
    show_vectors: bool = False,
    r: int = 0,
    tile_edgecolor: str = "k",
    reg_tile_edgcolor: str = "r",
    r_alpha: float = 0.3,
    cmap: list[str] = None,
    ...
    **kwargs,
    ...
    if cmap is None:
      ...
      cm = cmap
    if ax is None:
      ax = self.elements.plot(
        column="element_id", cmap=cm, figsize=figsize, **kwargs
      )
    else:
      self.elements.plot(
        ax=ax, column="element_id", cmap=cm, figsize=figsize, **kwargs
      )
    ...
    if r > 0:
      self.get_local_patch(r=r).plot(
        ax=ax, column="element_id", alpha=r_alpha, cmap=cm, **kwargs
      )
    if show_tile:
      self.tile.plot(ax=ax, ec=tile_edgecolor, lw=0.5, fc="#00000000", **kwargs)
