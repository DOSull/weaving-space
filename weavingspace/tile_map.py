...
class _TileGrid():
  ...
  def _get_area_to_tile(self, to_tile) -> geom.Polygon:
    ...
    poly = geom.Polygon(((bb[0], bb[1]), (bb[2], bb[1]),
                         (bb[2], bb[3]), (bb[0], bb[3])))
  ...
  def _get_extent(self) -> tuple[gpd.GeoSeries, geom.Point]:
    ...
    ext = affine.affine_transform(mrr_centre.buffer(radius), self.transform)
  ...
  def get_transforms(self) -> tuple[float]:
    ...
    vector_array = np.array([[v[0][0], v[1][0]], [v[0][1], v[1][1]]])
  ...
  def get_grid(self) -> gpd.GeoSeries:
    ...
    return gpd.GeoSeries(
        [p for p in pts if p.within(self.extent[0])]
...
class Tiling:
  ...
  def get_tiled_map(self, rotation:float = 0.,
    ...
    TODO: figure out how to make ragged_edges and prioritise_tiles options
...
class TiledMap:
  ...
  def render(self, **kwargs) -> Figure:
    ...
    if self.draft_mode:
      ...
      self.map.plot(ax = ax, column = "element_id", cmap = "tab20", **kwargs)
      return fig
    ...
    if self.legend:
      ...
      fig, axes = pyplot.subplots(
        1, 1, figsize = self.figsize, layout = "constrained", **kwargs)

    if self.variables is None:
      # get any floating point columns available
      default_columns = \
        self.map.select_dtypes(include = ("float64", "int64")).columns
  ...
  def plot_legend(self, ax: pyplot.Axes = None, **kwargs) -> None:
    ...
    context_tiles = gpd.GeoSeries(
      [g.simplify(1e-6) for g in context_tiles.geometry],
  ...
  def _get_legend_key_gdf(
    ...
    key_gdf = gpd.GeoDataFrame(
      ...
      crs = self.map.crs, geometry = gpd.GeoSeries(key_tiles))
