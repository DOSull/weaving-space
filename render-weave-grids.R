...
matrices_as_loom <- function(...) {
  ...
  if (length(matrices) == 1) {
    ...
    M_AB <- M1 %>% pracma::repmat(n = pracma::Lcm(nrow(M1), nA) %/% nrow(M1),
                          m = pracma::Lcm(ncol(M1), nA) %/% ncol(M1))
    M_BC <- M2 %>% pracma::repmat(n = pracma::Lcm(nrow(M2), nB) %/% nrow(M2),
                          m = pracma::Lcm(ncol(M2), nB) %/% ncol(M2))
    M_CA <- M3 %>% pracma::repmat(n = pracma::Lcm(nrow(M3), nC) %/% nrow(M3),
                          m = pracma::Lcm(ncol(M3), nC) %/% ncol(M3))
...
make_sf_from_coded_weave_matrix <- function(loom, spacing = 1, width = 1, margin = 0,
  ...
  for(i in 1:nrow(loom$indices)) {
    ...
    n_slices <- stringr::str_length(ids)          # number of slices required in each direction
