...
class WeaveUnit:
    ...
    Attributes:
        weave_unit: a GeoDataFrame of strand geometries.
    ...
    def __init__(self, weave_unit, tile, transform, type):
...
def get_weave_unit(weave_type:str = "plain", spacing:float = 10000, 
    ...
    """Returns a WeaveUnit object with the requested characteristics.
    
    Args:
        weave_type (str, optional): the type of weave pattern, one of "plain", 
            "twill", "basket", "this", "cube" or "hex". Defaults to "plain".
        spacing (float, optional): spacing of threads in the weave in units of 
            the CRS. Defaults to 10000.
        aspect (float, optional): width of strands relative to the spacing. 
            Defaults to 1.
        margin (float, optional): margin applied to 'shrink' strand elements, 
            relative to the spacing. Defaults to 0.
        n (tuple of ints): number of over-under strands in biaxial weaves. Only 
            one item is required in a plain weave. Twill and basket patterns 
            expect an even number of elements in the tuple. Defaults to (2, 2).
        strands (str, optional): specification of the strand labels along each
            axis. Defaults to "a|b|c".
        tie_up (numpy.ndarray, optional): used when type is "this" to specify a
            desired weave pattern. See: Glassner A, 2002, “Digital weaving. 1” 
            IEEE Computer Graphics and Applications 22(6) 108–118
            DOI: 10.1109/MCG.2002.1046635. Defaults to None.
        tr (numpy.ndarray, optional): used when type is "this" to specify the 
            treadling matrix. See: Glassner 2002. Defaults to None.
        th (numpy.ndarray, optional): used when type is "this" to specify the 
            threading matrix. See: Glassner 2002. Defaults to None.
        crs (int, optional): coordinate reference system. Usually an integer
            EPSG code, but any CRS object interpretable by geopandas will work. 
            Defaults to 3857 (for Web Mercator).

    Returns:
        WeaveUnit: a weave unit consistent with the supplied parameters.
    """  
...
def parameter_info(margin: float, aspect: float) -> None:
...
def get_tiling(unit: WeaveUnit, 
    ...
    tiles = chain(*[_translate_geoms(unit.weave_unit.geometry, dxdy) 
                    for dxdy in shifts])
